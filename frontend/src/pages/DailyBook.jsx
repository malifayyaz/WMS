...
export default function DailyBook() {
  ...
  useEffect(() => {
    if (jobWorkDeliveryDialogOpen && jobWorkDeliveryForm.sourceType === 'Annealing') {
      annealingAPI.getDeliverable({ materialType: 'Wire' })
        .then((res) => {
          setAnnealedWireOptions(res.data.data || []);
        })
        .catch(() => setAnnealedWireOptions([]));
    }
  }, [jobWorkDeliveryDialogOpen, jobWorkDeliveryForm.sourceType]);
  ...
  const openPersonalPaymentQuickDialog = async () => {
    try {
      ...
      chequesAPI.getInHand().then((resChq) => setInHandChequesList(resChq.data.data || [])).catch(() => {});
  ...
  const handleSaveParty = async () => {
    ...
    try {
      if (mainTab === 3) {
        ...
        else if (partyForm.alsoCustomer) payload.alsoProcessingCustomer = true;
        ...
        if (mainTab === 1 || mainTab === 2 || mainTab === 5) {
  ...
  const openJobWorkDeliveryEdit = (jobWork, delivery) => {
    ...
    setJobWorkDeliveryForm({
      ...
      sourceType: delivery.sourceType || 'Processing',
      sourceAnnealingId: delivery.sourceAnnealingId ? String(delivery.sourceAnnealingId) : '',
    });
    setJobWorkDeliveryDialogOpen(true);
  };
  ...
  const handleSaveJobWorkDelivery = async () => {
    ...
    if (jobWorkDeliveryForm.sourceType === 'Annealing' && !jobWorkDeliveryForm.sourceAnnealingId) {
      setSnack({ open: true, message: 'Select an annealed stock batch', severity: 'error' });
      return;
    }
    try {
      const payload = {
        ...
        sourceType: jobWorkDeliveryForm.sourceType,
        sourceAnnealingId: jobWorkDeliveryForm.sourceAnnealingId || undefined,
      };
    ...
    <Box>
      ...
      >
        ...
        <DialogContent sx={{ overflowY: 'auto' }}>
          ...
          />
          <FormControl fullWidth margin="dense">
            <InputLabel>Delivery Source</InputLabel>
            <Select
              value={jobWorkDeliveryForm.sourceType || 'Processing'}
              label="Delivery Source"
              onChange={(e) => setJobWorkDeliveryForm((f) => ({ ...f, sourceType: e.target.value, sourceAnnealingId: '' }))}
              disabled={!!jobWorkDeliveryEdit}
            >
              <MenuItem value="Processing">Direct Processing Pool</MenuItem>
              <MenuItem value="Annealing">Annealed Stock (Deduct from factory annealing)</MenuItem>
            </Select>
          </FormControl>
          {jobWorkDeliveryForm.sourceType === 'Annealing' && (
            <FormControl fullWidth margin="dense" required>
              <InputLabel>Select Annealed Batch</InputLabel>
              <Select
                value={jobWorkDeliveryForm.sourceAnnealingId || ''}
                label="Select Annealed Batch"
                onChange={(e) => setJobWorkDeliveryForm((f) => ({ ...f, sourceAnnealingId: e.target.value }))}
                disabled={!!jobWorkDeliveryEdit}
              >
                {annealedWireOptions.map((opt) => (
                  <MenuItem key={opt._id} value={opt._id}>
                    {opt.entryType === 'Send' ? 'Sent' : 'Arrived'} on {formatDate(opt.date)} — Wire {opt.wireNumber || 'N/A'} — {opt.remainingWeightKg} kg available (Batch {opt._id.slice(-4)})
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
        ...
        <DialogContent sx={{ overflowY: 'auto' }}>
          ...
          {mainTab === 3 ? (
            <>
              ...
              <Alert severity="info" sx={{ mt: 1.5 }}>
                Same person can also be a Customer (they buy wire or give coil). Ledgers stay separate, with a Combined view.
              </Alert>
              {partyForm.linkedCustomerId ? (
                <>
                  <FormControl fullWidth margin="dense">
                    ...
                    <Select
                      ...
                      onChange={(e) => setPartyForm((f) => ({ ...f, linkedCustomerId: e.target.value, alsoCustomer: false, unlinkCustomer: false }))}
                    >
                      {customers.map((c) => (
                ...
                <>
                  <FormControlLabel
                    control={(
                      <Checkbox
                        checked={!!partyForm.alsoCustomer}
                        onChange={(e) => setPartyForm((f) => ({
                          ...f,
                          alsoCustomer: e.target.checked,
                          linkedCustomerId: e.target.checked ? '' : f.linkedCustomerId,
                        }))}
                      />
                    )}
                    label="Also create as Customer (same person)"
                  />
                  {!partyForm.alsoCustomer && (
                    <FormControl fullWidth margin="dense">
                      <InputLabel>Or link existing Customer</InputLabel>
                      <Select
                        value={partyForm.linkedCustomerId || ''}
                        label="Or link existing Customer"
                        onChange={(e) => setPartyForm((f) => ({ ...f, linkedCustomerId: e.target.value, alsoCustomer: false }))}
                      >
                        <MenuItem value="">None</MenuItem>
                        {customers.map((c) => (
                          <MenuItem key={c._id} value={c._id}>{c.name}</MenuItem>
              ...
              <>
                ...
                <FormControl fullWidth size="small">
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      const m = e.target.value;
                      setPersonalPaymentForm({ ...personalPaymentForm, paymentMethod: m });
                      if (m === 'Cheque') {
                        chequesAPI.getInHand().then((resChq) => setInHandChequesList(resChq.data.data || [])).catch(() => {});
                      }
                    }}
                ...
                {personalPaymentForm.paymentMethod === 'Cheque' && (
                  <Box sx={{ p: 1.5, borderRadius: 2, bgcolor: 'rgba(25, 118, 210, 0.08)', border: '1px solid rgba(25, 118, 210, 0.2)' }}>
                    ...
                    <FormControl fullWidth size="small" margin="dense">
                      <InputLabel>Cheque Source</InputLabel>
                      <Select
                        value={personalPaymentForm.isEndorsedCheque ? 'Customer Cheque' : personalPaymentForm.chequeType}
                        label="Cheque Source"
                        onChange={(e) => {
                          const val = e.target.value;
                          if (val === 'Customer Cheque') {
                            setPersonalPaymentForm((f) => ({ ...f, isEndorsedCheque: true, chequeType: 'Customer Cheque' }));
                            chequesAPI.getInHand().then((res) => setInHandChequesList(res.data.data || [])).catch(() => {});
                          } else {
                            setPersonalPaymentForm((f) => ({ ...f, isEndorsedCheque: false, sourceChequeId: '', chequeType: val }));
                          }
                        }}
                      >
                        <MenuItem value="Customer Cheque">Customer Cheque (Passed / Endorsed)</MenuItem>
                    ...
                    {personalPaymentForm.isEndorsedCheque ? (
                      <>
                        <FormControl fullWidth size="small" margin="dense">
                          ...
                          <Select
                            ...
                            onChange={(e) => {
                              const chqId = e.target.value;
                              if (!chqId) {
                                setPersonalPaymentForm((f) => ({ ...f, sourceChequeId: '' }));
                                return;
                              }
                              const chosen = inHandChequesList.find((c) => String(c._id) === String(chqId));
                              setPersonalPaymentForm((f) => ({
                                ...f,
                                sourceChequeId: chqId,
                                amount: chosen ? String(chosen.amount) : f.amount,
                                chequeNumber: chosen ? chosen.chequeNumber : f.chequeNumber,
                        ...
                        <TextField
                          label="Drawer Bank Name *"
                          value={personalPaymentForm.chequeBank || personalPaymentForm.bankName}
                          onChange={(e) => setPersonalPaymentForm({ ...personalPaymentForm, chequeBank: e.target.value, bankName: e.target.value })}
                          fullWidth
                          size="small"
                          margin="dense"
                          placeholder="e.g. HBL, MCB, MBL"
                        ...
                        <TextField
                          label="Cheque Date (Maturity)"
                          type="date"
                          value={personalPaymentForm.chequeDate || personalPaymentForm.paymentDate}
                          onChange={(e) => setPersonalPaymentForm({ ...personalPaymentForm, chequeDate: e.target.value })}
                          fullWidth
                          size="small"
                          margin="dense"
                          InputLabelProps={{ shrink: true }}
